...
)
{
	...
	if (chr < 0x80) {	/* ASCII */
		...
		if (dir) {		/* OEM code to unicode */
			hi =UNIGBK_BLOCK2_SIZE / 4 - 1;
			readAddr = UNIGBK_BLOCK2_ADDR;
		} else {		/* Unicode to OEM code */
			hi =UNIGBK_BLOCK1_SIZE / 4 - 1;
			readAddr = UNIGBK_BLOCK1_ADDR;
		}
		li = 0;
		for (n = 16; n; n--) {
			i = li + (hi - li) / 2;
			SPI_FLASH_BufferRead((uint8_t*)buff, readAddr + i * 4, 4);
			if (chr == buff[0]) break;
			if (chr > buff[0])
				li = i;
			else
